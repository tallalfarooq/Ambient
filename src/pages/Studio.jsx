          ...
          <div className="flex items-center justify-between mb-4">
            {STEPS.map((label, i) => (
              <div key={label} className="flex items-center gap-2">
                ...
                <span className={`text-xs hidden sm:block transition-colors ${i === step ? "text-white/80" : "text-white/25"}`}>
                  {label}
                </span>
                {i < STEPS.length - 1 && (
                  <div
                    className={`flex-1 h-px mx-2 transition-all ${i < step ? "bg-violet-500" : "bg-white/10"}`}
                    style={{ width: 24 }}
                  />
                )}
              </div>
