...
function Ticker() {
  ...
  return (
    <div className="overflow-hidden border-b border-white/6 py-4">
      <div style={{ display: "flex", gap: 48, animation: "ticker 26s linear infinite", width: "max-content" }}>
        {items.map((t, i) => (
    ...
    <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", gap: "24px 48px", padding: "40px 24px", borderTop: "1px solid rgba(255,255,255,0.06)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
      {items.map((item, i) => (
        <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          <span style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-1px", background: "linear-gradient(135deg, #c9a96e, #d4756b, #9b7ec8)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            {item.number}
          </span>
          <span style={{ fontSize: 11, fontWeight: 500, letterSpacing: "0.05em", color: "#5a5550" }}>{item.label}</span>
        </div>
      ))}
    </div>
  );
...
function StyleCard({ s, i }) {
  return (
    <motion.div
      ...
      transition={{ delay: i * 0.06, duration: 0.5 }} viewport={{ once: true }} whileHover={{ y: -8 }}
    ...
    >
      ...
      <span
        ...
        style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.15)", color: "#fff" }}
      >{s.badge}</span>
...
function TestimonialCard({ t, i }) {
    ...
    return (
      ...
      >
        <div className="p-8 sm:p-12">
          <div className="text-base mb-5" style={{ color: "#C9963A", letterSpacing: 3 }}>{stars}</div>
        ...
        <div className="relative hidden sm:block overflow-hidden" style={{ minHeight: 280 }}>
          <img src="https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?w=800&q=85&auto=format&fit=crop" alt="AI redesigned living room" className="absolute inset-0 w-full h-full object-cover" />
          <div className="absolute inset-0" style={{ background: "linear-gradient(to right, rgba(26,26,26,0.35), transparent)" }} />
        </div>
      </motion.div>
    );
  }
  return (
    <motion.div
      ...
      style={{ background: "#1a1a1a" }}
    >
      <div className="text-sm mb-4" style={{ color: "#C9963A", letterSpacing: 3 }}>{stars}</div>
...
export default function Home() {
  ...
  const STEPS = [
    { n: "01", Icon: Upload,      title: t("step1_title"), desc: t("step1_desc") },
    { n: "02", Icon: Palette,     title: t("step2_title"), desc: t("step2_desc") },
    { n: "03", Icon: ShoppingBag, title: t("step3_title"), desc: t("step3_desc") },
  ];

  return (
    <div className="min-h-screen text-white overflow-x-hidden" style={{ background: "#0A0A0B" }}>
      <style>{`
      ...
      <section id="hero" className="relative flex items-center min-h-screen overflow-hidden" style={{ paddingTop: 0 }}>
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          <div className="absolute w-[500px] h-[500px] rounded-full"
            style={{ bottom: -50, left: "20%", background: "radial-gradient(circle, rgba(201,150,58,0.06), transparent 70%)", filter: "blur(100px)", animation: "floatOrb 14s ease-in-out infinite", animationDelay: "-5s" }} />
        </div>
        ...
        <div className="relative z-10 w-full lg:w-1/2 px-8 sm:px-16 py-24 flex flex-col justify-center">
          <motion.div initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9 }}>
            <div className="inline-flex items-center gap-2 rounded-full px-4 py-2 mb-8 text-xs font-semibold tracking-widest uppercase"
              style={{ background: "rgba(27,143,160,0.1)", border: "1px solid rgba(27,143,160,0.25)", color: "#1B8FA0" }}>
              <Sparkles className="w-3 h-3" />
              {t("hero_badge")}
            </div>

            <h1 className="font-bold leading-[1.05] mb-6" style={{ fontSize: "clamp(40px, 5.5vw, 76px)", letterSpacing: "-3px" }}>
              {t("hero_title_1")}{" "}
            ...
            <p className="mb-10 max-w-lg leading-relaxed" style={{ fontSize: "clamp(15px, 1.6vw, 18px)", color: "#8B9A9D" }}>
              {t("hero_subtitle")}
            </p>

            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-10">
              <button onClick={handleStart} className="group flex items-center gap-2.5 font-semibold transition-all duration-300 hover:opacity-90"
                style={{ background: "linear-gradient(135deg, #1B8FA0, #C9963A)", color: "#0A0A12", fontSize: 16, padding: "16px 36px", borderRadius: 100, boxShadow: "0 8px 32px rgba(27,143,160,0.35)" }}>
            ...
            <div className="flex flex-wrap items-center gap-4 sm:gap-6">
              ...
              <div className="text-sm" style={{ color: "#4A5568" }}>{t("hero_social_rooms")}</div>
            </div>
          </motion.div>
        </div>

        <div className="hidden lg:block absolute right-0 top-0 bottom-0" style={{ width: "52%" }}>
          <RoomVideoShowcase />
        </div>
      </section>
      ...
      <section id="how-it-works" className="py-28 px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-16">
            <div className="text-xs font-bold tracking-widest uppercase mb-4" style={{ color: "#1B8FA0" }}>{t("how_label")}</div>
            <h2 className="font-bold leading-tight mb-5" style={{ fontSize: "clamp(32px, 4vw, 56px)", letterSpacing: "-2px" }}>
              {t("how_title_1")} <br />{t("how_title_2")}
            </h2>
            <p className="text-lg max-w-md" style={{ color: "#8B9A9D" }}>{t("how_subtitle")}</p>
          </motion.div>

          <div className="grid grid-cols-1 sm:grid-cols-3 rounded-3xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.07)" }}>
            {STEPS.map((s, i) => (
              <motion.div
                key={s.n} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.12 }} viewport={{ once: true }}
              ...
              >
                <div className="text-6xl font-black mb-8 leading-none"
                  style={{ letterSpacing: "-4px", background: "linear-gradient(180deg, rgba(27,143,160,0.3), rgba(27,143,160,0.03))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                  {s.n}
                </div>
                <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-5 transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-6"
                  style={{ background: "rgba(27,143,160,0.08)", border: "1px solid rgba(27,143,160,0.2)" }}>
      ...
      {/* ── Before / After ────────────────────────────────────────────────── */}
      <section id="transformation" className="py-28 px-6"
        style={{ background: "#0D0D0F", borderTop: "1px solid rgba(255,255,255,0.04)", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-14">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <div className="text-xs font-bold tracking-widest uppercase mb-4" style={{ color: "#1B8FA0" }}>{t("transform_label")}</div>
              <h2 className="font-bold leading-tight" style={{ fontSize: "clamp(32px, 4vw, 52px)", letterSpacing: "-2px" }}>
                {t("transform_title_1")} <br />{t("transform_title_2")}
              </h2>
            </motion.div>
            <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
              className="text-base max-w-sm lg:text-right" style={{ color: "#8B9A9D" }}>
      ...
      <section id="styles" className="py-28 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-14">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              <div className="text-xs font-bold tracking-widest uppercase mb-4" style={{ color: "#1B8FA0" }}>{t("styles_label")}</div>
              <h2 className="font-bold leading-tight" style={{ fontSize: "clamp(32px, 4vw, 52px)", letterSpacing: "-2px" }}>
                {t("styles_title_1")} <br />{t("styles_title_2")}
              </h2>
            </motion.div>
            <button onClick={handleStart} className="text-sm font-medium flex-shrink-0 px-5 py-3 rounded-full transition-all duration-300 hover:border-white/20 hover:text-white"
      ...
      <section id="testimonials" className="py-28 px-6"
        style={{ background: "#0D0D0F", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-14">
            <div className="text-xs font-bold tracking-widest uppercase mb-4" style={{ color: "#1B8FA0" }}>{t("testimonials_label")}</div>
            <h2 className="font-bold leading-tight" style={{ fontSize: "clamp(32px, 4vw, 52px)", letterSpacing: "-2px" }}>{t("testimonials_title")}</h2>
          </motion.div>
      ...
      {/* ── Final CTA ─────────────────────────────────────────────────────── */}
      <section id="cta" className="py-40 px-6 text-center relative overflow-hidden"
        style={{ borderTop: "1px solid rgba(255,255,255,0.04)", background: "#0A0A0B" }}>
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: "radial-gradient(ellipse 60% 60% at 50% 50%, rgba(27,143,160,0.07), transparent 70%)" }} />
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative z-10 max-w-3xl mx-auto">
          ...
          <p className="mb-12 text-lg" style={{ color: "#8B9A9D" }}>{t("cta_subtitle")}</p>
          <button onClick={handleStart} className="inline-flex items-center gap-3 font-semibold transition-all duration-300 hover:opacity-90"
            style={{ background: "linear-gradient(135deg, #1B8FA0, #C9963A)", color: "#0A0A12", fontSize: 18, padding: "20px 44px", borderRadius: 100, boxShadow: "0 8px 32px rgba(27,143,160,0.3)" }}>
        ...
        {showAuthModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92 }}
              ...
              <p className="text-sm mb-8 leading-relaxed" style={{ color: "rgba(255,255,255,0.45)" }}>{t("auth_subtitle")}</p>
              <div className="flex flex-col gap-3">
                <button onClick={() => base44.auth.redirectToLogin(window.location.origin + createPageUrl("Studio"))}
              ...
              <p className="text-xs mt-6" style={{ color: "rgba(255,255,255,0.25)" }}>{t("auth_no_card")}</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
